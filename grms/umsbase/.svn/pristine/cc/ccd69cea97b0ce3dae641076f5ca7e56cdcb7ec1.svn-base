...
public class SimpleJdbcTemplate {
	...
	public Object findForObject(final String sql,Class clazz,Map parameters){
		try{
			...
			Assert.notNull(clazz,"集合中对象类型不能为空!");
			if(parameters!=null){
				//rn jdbcTemplate.queryForObject(sql, resultBeanMapper(clazz), parameters);
	...
	public long findForLong(final String sql,Map parameters){
		try{
			...
			if(parameters!=null){
				...
				Number number = this.jdbcTemplate.queryForObject(sql,Long.class,parameters);
				long totalCount = (number!=null?number.longValue():0); 
				return totalCount;
			}else{
				//return jdbcTemplate.queryForLong(sql);
				Number number = this.jdbcTemplate.queryForObject(sql,Long.class);
				long totalCount = (number!=null?number.longValue():0); 
				return totalCount;
			}
		}catch (Exception e) {
	...
	public int executeForObject(final String sql,Object bean){
		...
		if(bean!=null){
			...
			return jdbcTemplate.update(sql, bean);
		}else{
