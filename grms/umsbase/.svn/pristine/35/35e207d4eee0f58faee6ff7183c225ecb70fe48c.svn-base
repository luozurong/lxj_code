...
public class ExportexcelWithAdvertisement {
	 ...
	 * @param list
	 */
	public static void generateclsjExcelFor2003(String fileName, List<AdMaterialTotalPage> list,
			HttpServletResponse response,int baseLong){
		HSSFWorkbook workbook2003 = new HSSFWorkbook();
		HSSFSheet sheet = workbook2003.createSheet("广告数据统计表");
		int addLong = 0;
		int qtyLong = 0;
		//对列设置宽度为80像素
	    ...
	    for(int i = 0 ; i < qtyLong ; i++){
			...
			case 2:
				...
				break;
			case 3:
				cell.setCellValue("播放次数");
				break;
			case 4:
				cell.setCellValue("用户观看次数");
				break;
			default:
				break;
			}
	    ...
	    if(list != null && list.size() > 0) {
	    	...
	    	for(int j = 0 ; j < list.size() ; j++){
	    		AdMaterialTotalPage data = list.get(j);
	    		//生成行
	    		HSSFRow row = sheet.createRow(j+1);
				
				HSSFCell descript = row.createCell(0);
				descript.setCellValue(data.getName());
				descript.setCellStyle(commonStyle);
				
				
				HSSFCell address = row.createCell(1);
				address.setCellValue(data.getCustomerName());
				address.setCellStyle(commonStyle);
				
				HSSFCell familyCount = row.createCell(2);
				familyCount.setCellValue(data.getPushTime());
				familyCount.setCellStyle(commonStyle);
				
				HSSFCell terminalCount = row.createCell(3);
				terminalCount.setCellValue(data.getPlayCount());
				terminalCount.setCellStyle(commonStyle);
				
				if(baseLong==5){
					HSSFCell grandCardCount = row.createCell(4);
					grandCardCount.setCellValue(data.getUserSawTimes());
