...
public class ByteOps {
	...
	public static int makeInt(byte b3, byte b2, byte b1, byte b0) {
		return ((b3 << 24) | (b2 << 16) | (b1 << 8) | (b0 & 0xff)) & 0xffffffff;
	}
	 ...
	 * @param mainBytes 需要添加数据的数组
	 * @param addedBytes 被添加数据的数据
	 * @param off mainBytes添加开始位置
	 * @param length addedBytes需要添加数据的长度
	 */
	...
	public static void addByteArray(byte[] mainBytes, byte addedByte, int off){
	  ...
	  /**
	    ...
	    */
	    ...
	    public static String bytesToNormalString(byte[] bytes) throws UnsupportedEncodingException{
	    	String detailString = new String(bytes, "GBK");
	    ...
	    public static Number makeNumFromBytes(byte[] body, int position, int length){
	    	...
	    	case 2://INT
	    		...
	    		break;
	    	case 4://LONG
	    		result = makeLong(body[position+3], body[position+2], body[position+1], body[position]);
	    		break;
	    	default://biginteger
	    ...
	    public static String makeNormalStringFromBytes(byte[] body, int position, int length) throws UnsupportedEncodingException{
	    	byte[] bigBytes = cpByteArray(body, position, length);
	    	return bytesToNormalString(bigBytes);
	    }
	    
	    public static float makeFloat(byte[] bytes, ByteOrder byteOrder){
	    ...
	    public static float makeFloat(byte[] bytes, int position, int length){
	    	byte[] bigBytes = cpByteArray(bytes, position, length);
	    	ByteBuffer buf=ByteBuffer.allocateDirect(bigBytes.length);
	    ...
	    public static double makeDouble(byte[] bytes, ByteOrder byteOrder){
	    	ByteBuffer buf=ByteBuffer.allocateDirect(bytes.length);
	    	buf=buf.order(byteOrder);
	    	buf.put(bytes);
	    	buf.rewind();
	    	double f2=buf.getDouble();
	    	return f2;
	    }
		...
		public static void main(String[] args){
			...
			//System.out.println(bytesToNormalString(bytes));
		}
