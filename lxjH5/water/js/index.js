...
var time_stamp;
var onloadStatus = 0;//http://114.215.94.79:8090/mms/html5/water/index.htm?token=token&areaSerial=areaSerial&organizationSeq=4444&householdSerial=xxx&clientType=ios&commodityId=aaaa
...
if(url.indexOf("?") != -1) {
	...
	advStatus = key['advStatus'];//广告进入列表标识
	
	var indexStatus=sessionStorage.getItem("indexStatus");
	if (indexStatus!="1") {
		commodityId = key['commodityId'];
		if(isCondition(commodityId)) {
			sessionStorage.setItem("commodityId", commodityId);
			sessionStorage.setItem("indexStatus", "1");
		}
	}
}
if(isCondition(token)) {
	sessionStorage.setItem("token", token);
	var cId=sessionStorage.getItem("commodityId");
	if (isCondition(cId)) {
		sessionStorage.setItem("status","1");
	}
	if (isCondition(advStatus)) {
		goStatus=1;
...
function init() {
	...
	if(isCondition(commodityId)) {
		...
		var time_stamp = Date.parse(new Date());
		var waterdata = "{\"body\":{\"commodityId\":\"" + commodityId + "\",\"moduleName\":\"" + moduleName + "\",\"areaSeq\":\"" + areaOrgaSeq + "\"},\"header\":{\"token\":\"" + token + "\",\"time_stamp\":\"" + time_stamp + "\"}}";
		$.ajax({
			type: "get",
