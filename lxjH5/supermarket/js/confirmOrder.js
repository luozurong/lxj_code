	...
	$.ajax({
		...
		success: function(data) {
			...
			if(data.result == 0) {
                ...
                var totalPrice=doubleValue(data.totalPrice);
                sessionStorage.setItem("totalPriceF",totalPrice);
				if(sessionStorage.getItem("freightPriceF")>0){
					totalPrice=doubleValue(Number(totalPrice)+Number(sessionStorage.getItem("freightPriceF")));
				}
				...
				if(totalReducePrice<=0){
					//$(".text-payment").css("line-height","50px");
                    $(".text-totalizer").html("¥"+totalPrice);
					$(".text-reduction").hide();
				}
				//if(totalReducePrice<=0){
				//	$(".text-payment").css("line-height","50px");
				//	$(".text-totalizer,.text-reduction").hide();
				//}
				//if(totalReducePrice<=0){
				//	//$(".text-payment").css("line-height","50px");
				//	$(".text-reduction").hide();
				//}
				if(totalReducePrice>0){
					$(".text-payment").css("line")
					$(".text-totalizer,.text-reduction").show();
			...
			$("#product").on("click", function() {
				...
				if(isCondition(shoppingCartId != null && shoppingCartId != "" && !(typeof(shoppingCartId) === undefined))) {
					showActivitySpecial(host + "/mms/html5/supermarket/productList.htm?shoppingCartId=" + shoppingCartId, "商品清单", 2, bean);
				} else {
					showActivitySpecial(host + "/mms/html5/supermarket/productList.htm?skuId=" + skuId + "&commodityId=" + commodityId + "&commodityCount=" + commodityCount, "商品清单", 2, bean);
				}
