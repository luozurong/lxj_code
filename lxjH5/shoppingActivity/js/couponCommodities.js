...
var vue = new Vue({
	...
	methods: {
		...
		getCoupon: function(couponId) {
			...
			_hmt.push(['_trackEvent', "2049", "1", "点我抢按钮"]);
			if(this.isVisitor) {
				this.gologinFun();
				return false;
			}
		...
		selectSku: function(id, baiduText, indexN) {
			window.event.cancelBubble = true;
			_hmt.push(['_trackEvent', "2049", "2", "加入购物车"]);
			_hmt.push(['_trackEvent', "2049", indexN, baiduText + "—加入购物车"]);
			//游客状态先走授权绑定流程
			if(this.isVisitor) {
		...
		goShoppingcart: function() {
			...
			_hmt.push(['_trackEvent', "2049", "3", "购物车图标点击次数"]);
			if(this.isVisitor) {
				this.gologinFun();
				return false;
			}
		...
		goDetail: function(id, nameP, indexP) {
			window.event.cancelBubble = true;
			_hmt.push(['_trackEvent', "2049", indexP, nameP + "—商品详情"]);
