(function($, window) {
	...
	proto._initImgData = function(itemData, imgEl) {
		...
		imgEl.style.webkitTransform = 'translate3d(0,0,0) scale(' + 1 + ')';
	};
	...
	proto._loadItem = function(index, isOpening) { 
		...
		if (isOpening) {
			...
			imgEl.style.webkitTransform = 'translate3d(' + posi.x + 'px,' + posi.y + 'px,0) scale(' + 1 + ')';
			imgEl.offsetHeight;
		}
	...
	proto.refresh = function(index, groupArray) {
		...
		for (var i = 0; from < to; from++, i++) {
			...
			var style = '';
			if (itemData.sWidth) {
				style = '-webkit-transform:translate3d(0,0,0) scale(' + 1 + ');transform:translate3d(0,0,0) scale(' + 1 + ')';
			}
	...
	proto.open = function(index, group) {
		...
		switchFullScreen(true);//全屏需要ios出版本才可以开放
	};
	proto.close = function(index, group) {
		switchFullScreen(false);//全屏需要ios出版本才可以开放
		if (!this.isShown()) {
			return;
		}
		...
		if (imgEl) {
			...
			if (sTop > window.innerHeight || sLeft > window.innerWidth || sTop < 0 || sLeft < 0) { //out viewport
				...
				imgEl.style.webkitTransitionDuration = '0.5s';
				imgEl.style.webkitTransform = 'scale(' + 1 + ')';
			} else {
				if (this.options.zoom) {
...
//		this.empty();
	...
	//	appAlert("overflow",$("body").css("overflow"));
	};
