	...
	MobileSelectArea.prototype = {
		...
		getData: function() {
			...
			if (typeof this.settings.data == "object") {
			    console.debug("dataObject:"+this.settings.data);
				console.debug("dataJSON:"+JSON.stringify(this.settings.data));
		...
		submit: function() {
			...
			console.debug("hd_area:"+this.value.join(','));
			this.trigger.next().next(':hidden').val(this.value.join(','));
