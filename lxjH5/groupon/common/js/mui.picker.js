...
(function($, window, document, undefined) {
	...
	Picker.prototype.endScroll = function() {
		...
		if (self.list.angle < self.beginAngle) {
			...
			index = parseInt((self.list.angle / self.itemAngle).toFixed(0));
		...
		if(item != undefined && item.value != undefined && item.next != undefined){
			if(item.next == 'city'){
				sessionStorage.setItem('provinceIndex',index);
				sessionStorage.setItem('cityIndex',0);
				setTimeout(function(){
					getAddressdate(2,item.value,item.next);
				},200);
