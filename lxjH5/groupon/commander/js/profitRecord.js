...
var vue = new Vue({
	...
	methods: {
		getData: function(pageSize,pageNum) {
			...
			axios.post(this.mmsHost + "/mms/group/getProfitInfo", params).then(function(res) {
				console.log(res);
				onscrollStatus = true;
				that.recordData=res.data;
				var list = res.data.list ? res.data.list : [];
				that.recordList=that.recordList.concat(list);
				that.presentTotal = Math.ceil(that.recordList.length / 10) * 10;	
			}, function(res) {})						
		},
